$(() => (async () => {
	if (mw.config.get("skin") == 'moeskin' && mw.config.get("wgNamespaceNumber") != -1) {
		await mw.loader.using(["mediawiki.api"]);

		const PAGE = mw.config.get("wgPageName");
		let indicators = Object.fromEntries(Object.entries(await getIndicatorEntries(PAGE)).filter(([k, ]) => !/noteTA-.+/g.test(k)));
	...
	async function getIndicatorEntries(page) {
		...
		return await API.get({
			...
			for (const INDICATOR of res.parse.indicators) {
